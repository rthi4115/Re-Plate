    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <div className="bg-white border-b sticky top-[73px] z-20">
        <div className="max-w-6xl mx-auto px-6 flex gap-8">
          <button 
            className={`py-4 font-semibold text-sm border-b-2 transition-colors ${activeTab === 'map' ? 'border-primary text-primary' : 'border-transparent text-gray-500 hover:text-gray-800'}`}
            onClick={() => setActiveTab('map')}
          >
            Map View
          </button>
          <button 
            className={`py-4 font-semibold text-sm border-b-2 transition-colors ${activeTab === 'list' ? 'border-primary text-primary' : 'border-transparent text-gray-500 hover:text-gray-800'}`}
            onClick={() => setActiveTab('list')}
          >
            Available List
          </button>
          <button 
            className={`py-4 font-semibold text-sm border-b-2 transition-colors ${activeTab === 'claims' ? 'border-primary text-primary' : 'border-transparent text-gray-500 hover:text-gray-800'}`}
      ...
      <main className="flex-grow flex flex-col max-w-6xl mx-auto w-full p-6">
        {activeTab === 'map' && (
          <div className="bg-white p-2 rounded-[14px] shadow-md flex-grow min-h-[500px]">
                ...
                return (
                  <Marker key={listing.id} position={[coord.lat, coord.lng]}>
                    <Popup className="rounded-xl">
                      ...
                      <div className="text-sm mb-2">{listing.quantity} • {listing.servings} Servings</div>
                      <button 
                        onClick={() => handleClaim(listing.id)}
                        className="bg-primary text-white px-3 py-1.5 rounded-md text-xs font-bold w-full"
                      >
                        Claim Listing
                      </button>
                    </Popup>
              ...
              availableListings.map(listing => (
                <ListingCard 
                  ...
                  actionElement={
                    <button 
                      onClick={() => handleClaim(listing.id)}
                      className="w-full bg-white border border-primary text-primary hover:bg-primary hover:text-white transition-colors duration-200 py-2 rounded-lg font-semibold text-sm"
                    >
                      Claim & Deliver
                    </button>
                  }
